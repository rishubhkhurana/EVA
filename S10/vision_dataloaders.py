...
import torch
from albumentations import HorizontalFlip, HueSaturationValue, Rotate,RandomSizedCrop, Compose, Resize, Normalize,VerticalFlip,Cutout
...
def get_crop(size=32,padding=4):
    return RandomSizedCrop((size,size),size,size)
...
_transforms={'tensor':get_tensor,'rotate':get_rotate,'normalize':get_normalize,'hflip':partial(get_flip,'horizontal'),'vflip':partial(get_flip,'vertical'),'crop':get_crop,'cutout':get_cutout,'jitter':get_jitter}
