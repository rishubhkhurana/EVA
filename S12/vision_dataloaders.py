...
def create_transform(tfm_names,**kwargs):
    ...
    for t in tfm_names:
        ...
        if t=='rotate':
            ...
            if 'hsv_prob' in kwargs:
                params['p'] = kwargs['hsv_prob']
        else:
            tfms.append(_transforms[t]())

    return Compose(tfms)
        
_transforms={'tensor':get_tensor,'rotate':get_rotate,'normalize':get_normalize,'hflip':partial(get_flip,'horizontal'),'vflip':partial(get_flip,'vertical'),'crop':get_crop,'cutout':get_cutout,'hsv':get_hsv,'pad':get_padding}
